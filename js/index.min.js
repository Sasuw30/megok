/**
 * Minified by jsDelivr using Terser v5.39.0.
 * Original file: /npm/spessasynth_lib@4.0.18/dist/index.js
 *
 * Do NOT use SRI with dynamically generated files! More information: https://www.jsdelivr.com/using-sri-with-dynamic-files
 */
import {KeyModifier} from "spessasynth_core";
function fillWithDefaults(e, t) {
    return {
        ...t,
        ...e ?? {}
    }
}
var WorkletKeyModifierManagerWrapper = class {
    keyModifiers = [];
    synth;
    constructor(e) {
        this.synth = e
    }
    addModifier(e, t, s) {
        const n = new KeyModifier;
        n.gain = s?.gain ?? 1,
        n.velocity = s?.velocity ?? -1,
        n.patch = fillWithDefaults(s.patch ?? {}, {
            isGMGSDrum: !1,
            bankLSB: -1,
            bankMSB: -1,
            program: -1
        }),
        this.keyModifiers[e] ??= [],
        this.keyModifiers[e][t] = n,
        this.sendToWorklet("addMapping", {
            channel: e,
            midiNote: t,
            mapping: n
        })
    }
    getModifier(e, t) {
        return this.keyModifiers?.[e]?.[t]
    }
    deleteModifier(e, t) {
        this.sendToWorklet("deleteMapping", {
            channel: e,
            midiNote: t
        }),
        void 0 !== this.keyModifiers[e]?.[t] && (this.keyModifiers[e][t] = void 0)
    }
    clearModifiers() {
        this.sendToWorklet("clearMappings", null),
        this.keyModifiers = []
    }
    sendToWorklet(e, t) {
        const s = {
            type: e,
            data: t
        };
        this.synth.post({
            type: "keyModifierManager",
            channelNumber: -1,
            data: s
        })
    }
}
;
import {SpessaSynthCoreUtils} from "spessasynth_core";
var SoundBankManager = class {
    soundBankList;
    synth;
    constructor(e) {
        this.soundBankList = [],
        this.synth = e
    }
    get priorityOrder() {
        return this.soundBankList.map((e=>e.id))
    }
    set priorityOrder(e) {
        this.sendToWorklet("rearrangeSoundBanks", e),
        this.soundBankList.sort(((t,s)=>e.indexOf(t.id) - e.indexOf(s.id)))
    }
    async addSoundBank(e, t, s=0) {
        this.sendToWorklet("addSoundBank", {
            soundBankBuffer: e,
            bankOffset: s,
            id: t
        }, [e]),
        await this.awaitResponse();
        const n = this.soundBankList.find((e=>e.id === t));
        void 0 !== n ? n.bankOffset = s : this.soundBankList.push({
            id: t,
            bankOffset: s
        })
    }
    async deleteSoundBank(e) {
        this.soundBankList.length < 2 ? SpessaSynthCoreUtils.SpessaSynthWarn("1 sound bank left. Aborting!") : -1 !== this.soundBankList.findIndex((t=>t.id === e)) ? (this.sendToWorklet("deleteSoundBank", e),
        this.soundBankList = this.soundBankList.filter((t=>t.id !== e)),
        await this.awaitResponse()) : SpessaSynthCoreUtils.SpessaSynthWarn(`No sound banks with id of "${e}" found. Aborting!`)
    }
    async awaitResponse() {
        return new Promise((e=>this.synth.awaitWorkerResponse("soundBankManager", e)))
    }
    sendToWorklet(e, t, s=[]) {
        const n = {
            type: "soundBankManager",
            channelNumber: -1,
            data: {
                type: e,
                data: t
            }
        };
        this.synth.post(n, s)
    }
}
  , SynthEventHandler = class {
    timeDelay = 0;
    events = {
        noteOff: new Map,
        noteOn: new Map,
        pitchWheel: new Map,
        controllerChange: new Map,
        programChange: new Map,
        channelPressure: new Map,
        polyPressure: new Map,
        drumChange: new Map,
        stopAll: new Map,
        newChannel: new Map,
        muteChannel: new Map,
        presetListChange: new Map,
        allControllerReset: new Map,
        soundBankError: new Map,
        synthDisplay: new Map,
        masterParameterChange: new Map,
        channelPropertyChange: new Map
    };
    addEvent(e, t, s) {
        this.events[e].set(t, s)
    }
    removeEvent(e, t) {
        this.events[e].delete(t)
    }
    callEventInternal(e, t) {
        const s = this.events[e]
          , n = ()=>{
            s.forEach((s=>{
                try {
                    s(t)
                } catch (t) {
                    console.error(`Error while executing an event callback for ${e}:`, t)
                }
            }
            ))
        }
        ;
        this.timeDelay > 0 ? setTimeout(n.bind(this), 1e3 * this.timeDelay) : n()
    }
}
;
import {ALL_CHANNELS_OR_DIFFERENT_ACTION, DEFAULT_MASTER_PARAMETERS, DEFAULT_PERCUSSION, midiControllers, midiMessageTypes, SpessaSynthCoreUtils as util} from "spessasynth_core";
var BasicEffectsProcessor = class {
    input;
    output;
    constructor(e, t) {
        this.input = e,
        this.output = t
    }
    connect(e) {
        return this.output.connect(e)
    }
    disconnect(e) {
        return e ? this.output.disconnect(e) : this.output.disconnect()
    }
    delete() {
        this.input.disconnect(),
        this.output.disconnect()
    }
}
  , NODES_AMOUNT = 4
  , DEFAULT_DELAY = .03
  , DELAY_VARIATION = .013
  , STEREO_DIFF = .03
  , OSC_FREQ = .2
  , OSC_FREQ_VARIATION = .05
  , OSC_GAIN = .003
  , DEFAULT_CHORUS_CONFIG = {
    nodesAmount: NODES_AMOUNT,
    defaultDelay: DEFAULT_DELAY,
    delayVariation: DELAY_VARIATION,
    stereoDifference: STEREO_DIFF,
    oscillatorFrequency: OSC_FREQ,
    oscillatorFrequencyVariation: OSC_FREQ_VARIATION,
    oscillatorGain: OSC_GAIN
}
  , ChorusProcessor = class extends BasicEffectsProcessor {
    chorusLeft = [];
    chorusRight = [];
    constructor(e, t=DEFAULT_CHORUS_CONFIG) {
        super(e.createChannelSplitter(2), e.createChannelMerger(2)),
        this.update(t)
    }
    _config = DEFAULT_CHORUS_CONFIG;
    get config() {
        return this._config
    }
    update(e) {
        this.deleteNodes();
        const t = fillWithDefaults(e, DEFAULT_CHORUS_CONFIG);
        this._config = t;
        let s = t.oscillatorFrequency
          , n = t.defaultDelay;
        for (let e = 0; e < t.nodesAmount; e++)
            this.createChorusNode(s, n, this.chorusLeft, 0, this.output, 0, t),
            this.createChorusNode(s, n + t.stereoDifference, this.chorusRight, 1, this.output, 1, t),
            s += t.oscillatorFrequencyVariation,
            n += t.delayVariation
    }
    delete() {
        super.delete(),
        this.deleteNodes()
    }
    deleteNodes() {
        this.input.disconnect();
        for (const e of this.chorusLeft)
            e.delay.disconnect(),
            e.oscillator.disconnect(),
            e.oscillator.stop(),
            e.oscillatorGain.disconnect();
        for (const e of this.chorusRight)
            e.delay.disconnect(),
            e.oscillator.disconnect(),
            e.oscillator.stop(),
            e.oscillatorGain.disconnect();
        this.chorusLeft.length = 0,
        this.chorusRight.length = 0
    }
    createChorusNode(e, t, s, n, a, i, o) {
        const r = a.context
          , h = r.createOscillator();
        h.type = "sine",
        h.frequency.value = e;
        const l = r.createGain();
        l.gain.value = o.oscillatorGain;
        const c = r.createDelay();
        c.delayTime.value = t,
        h.connect(l),
        l.connect(c.delayTime),
        h.start(r.currentTime),
        this.input.connect(c, n),
        c.connect(a, 0, i),
        s.push({
            oscillator: h,
            oscillatorGain: l,
            delay: c
        })
    }
}
  , DEFAULT_SYNTH_CONFIG = {
    enableEventSystem: !0,
    oneOutput: !1,
    audioNodeCreators: void 0,
    initializeChorusProcessor: !0,
    initializeReverbProcessor: !0
};
import {SpessaSynthCoreUtils as SpessaSynthCoreUtils2} from "spessasynth_core";
var consoleColors = SpessaSynthCoreUtils2.consoleColors
  , rbCompressed = "";
import {SpessaSynthCoreUtils as SpessaSynthCoreUtils3} from "spessasynth_core";
var binaryString = atob(rbCompressed)
  , binary = new Uint8Array(binaryString.length);
for (let e = 0; e < binaryString.length; e++)
    binary[e] = binaryString.charCodeAt(e);
var reverbBufferBinary = SpessaSynthCoreUtils3.inflateSync(binary).buffer
  , DEFAULT_REVERB_CONFIG = {
    impulseResponse: void 0
}
  , ReverbProcessor = class extends BasicEffectsProcessor {
    isReady;
    conv;
    constructor(e, t=DEFAULT_REVERB_CONFIG) {
        const s = fillWithDefaults(t, DEFAULT_REVERB_CONFIG)
          , n = e.createConvolver();
        super(n, n),
        this.conv = n;
        const a = s.impulseResponse;
        a ? (n.buffer = a,
        this.isReady = new Promise((e=>e(a)))) : (this.isReady = e.decodeAudioData(reverbBufferBinary.slice(0)),
        this.isReady.then((e=>{
            n.buffer = e
        }
        ))),
        this._config = s
    }
    _config;
    get config() {
        return this._config
    }
    update(e) {
        const t = fillWithDefaults(e, DEFAULT_REVERB_CONFIG);
        t.impulseResponse && (this.conv.buffer = t.impulseResponse),
        this._config = t
    }
}
;
import {ChannelSnapshot, SynthesizerSnapshot} from "spessasynth_core";
var LibSynthesizerSnapshot = class extends SynthesizerSnapshot {
    chorusConfig;
    reverbConfig;
    constructor(e, t, s, n=DEFAULT_CHORUS_CONFIG, a=DEFAULT_REVERB_CONFIG) {
        super(e.map((e=>ChannelSnapshot.copyFrom(e))), t, s),
        this.reverbConfig = {
            ...a
        },
        this.chorusConfig = {
            ...n
        }
    }
    getCoreSnapshot() {
        return new SynthesizerSnapshot([...this.channelSnapshots],{
            ...this.masterParameters
        },[...this.keyMappings])
    }
}
  , DEFAULT_SYNTH_METHOD_OPTIONS = {
    time: 0
}
  , BasicSynthesizer = class {
    soundBankManager = new SoundBankManager(this);
    keyModifierManager = new WorkletKeyModifierManagerWrapper(this);
    eventHandler = new SynthEventHandler;
    context;
    channelProperties = [];
    presetList = [];
    sequencerCallbackFunction;
    isReady;
    reverbProcessor;
    chorusProcessor;
    post;
    worklet;
    _outputsAmount = 16;
    channelsAmount = this._outputsAmount;
    masterParameters = {
        ...DEFAULT_MASTER_PARAMETERS
    };
    resolveMap = new Map;
    renderingProgressTracker = new Map;
    constructor(e, t, s) {
        util.SpessaSynthInfo("%cInitializing SpessaSynth synthesizer...", consoleColors.info),
        this.context = e.context,
        this.worklet = e,
        this.post = t,
        this.isReady = new Promise((e=>this.awaitWorkerResponse("sf3Decoder", e)));
        const n = fillWithDefaults(s, DEFAULT_SYNTH_CONFIG);
        this.worklet.port.onmessage = e=>this.handleMessage(e.data),
        n.initializeReverbProcessor && (this.reverbProcessor = new ReverbProcessor(this.context),
        this.isReady = Promise.all([this.isReady, this.reverbProcessor.isReady]),
        this.worklet.connect(this.reverbProcessor.input, 0)),
        n.initializeChorusProcessor && (this.chorusProcessor = new ChorusProcessor(this.context),
        this.worklet.connect(this.chorusProcessor.input, 1));
        for (let e = 0; e < this.channelsAmount; e++)
            this.addNewChannelInternal(!1);
        this.channelProperties[DEFAULT_PERCUSSION].isDrum = !0,
        this.eventHandler.addEvent("newChannel", `synth-new-channel-${Math.random()}`, (()=>{
            this.channelsAmount++
        }
        )),
        this.eventHandler.addEvent("presetListChange", `synth-preset-list-change-${Math.random()}`, (e=>{
            this.presetList = [...e]
        }
        )),
        this.eventHandler.addEvent("masterParameterChange", `synth-master-parameter-change-${Math.random()}`, (e=>{
            this.masterParameters[e.parameter] = e.value
        }
        )),
        this.eventHandler.addEvent("channelPropertyChange", `synth-channel-property-change-${Math.random()}`, (e=>{
            this.channelProperties[e.channel] = e.property,
            this._voicesAmount = this.channelProperties.reduce(((e,t)=>e + t.voicesAmount), 0)
        }
        ))
    }
    _voicesAmount = 0;
    get voicesAmount() {
        return this._voicesAmount
    }
    get currentTime() {
        return this.context.currentTime
    }
    connect(e) {
        this.reverbProcessor?.connect(e),
        this.chorusProcessor?.connect(e);
        for (let t = 2; t < this.channelsAmount + 2; t++)
            this.worklet.connect(e, t);
        return e
    }
    disconnect(e) {
        if (!e)
            return this.reverbProcessor?.disconnect(),
            this.chorusProcessor?.disconnect(),
            void this.worklet.disconnect();
        this.reverbProcessor?.disconnect(e),
        this.chorusProcessor?.disconnect(e);
        for (let t = 2; t < this.channelsAmount + 2; t++)
            this.worklet.disconnect(e, t);
        return e
    }
    setLogLevel(e, t, s) {
        this.post({
            channelNumber: ALL_CHANNELS_OR_DIFFERENT_ACTION,
            type: "setLogLevel",
            data: {
                enableInfo: e,
                enableWarning: t,
                enableGroup: s
            }
        })
    }
    getMasterParameter(e) {
        return this.masterParameters[e]
    }
    setMasterParameter(e, t) {
        this.masterParameters[e] = t,
        this.post({
            type: "setMasterParameter",
            channelNumber: ALL_CHANNELS_OR_DIFFERENT_ACTION,
            data: {
                type: e,
                data: t
            }
        })
    }
    async getSnapshot() {
        return new Promise((e=>{
            this.awaitWorkerResponse("synthesizerSnapshot", (t=>{
                const s = new LibSynthesizerSnapshot(t.channelSnapshots,t.masterParameters,t.keyMappings,this.chorusProcessor?.config,this.reverbProcessor?.config);
                e(s)
            }
            )),
            this.post({
                type: "requestSynthesizerSnapshot",
                data: null,
                channelNumber: -1
            })
        }
        ))
    }
    addNewChannel() {
        this.addNewChannelInternal(!0)
    }
    setVibrato(e, t) {
        this.post({
            channelNumber: e,
            type: "setChannelVibrato",
            data: t
        })
    }
    connectIndividualOutputs(e) {
        if (e.length !== this._outputsAmount)
            throw new Error(`input nodes amount differs from the system's outputs amount!\n            Expected ${this._outputsAmount} got ${e.length}`);
        for (let t = 0; t < this._outputsAmount; t++)
            this.worklet.connect(e[t], t + 2)
    }
    disconnectIndividualOutputs(e) {
        if (e.length !== this._outputsAmount)
            throw new Error(`input nodes amount differs from the system's outputs amount!\n            Expected ${this._outputsAmount} got ${e.length}`);
        for (let t = 0; t < this._outputsAmount; t++)
            this.worklet.disconnect(e[t], t + 2)
    }
    disableGSNPRNParams() {
        this.setVibrato(ALL_CHANNELS_OR_DIFFERENT_ACTION, {
            depth: 0,
            rate: -1,
            delay: 0
        })
    }
    sendMessage(e, t=0, s=DEFAULT_SYNTH_METHOD_OPTIONS) {
        this._sendInternal(e, t, !1, s)
    }
    noteOn(e, t, s, n=DEFAULT_SYNTH_METHOD_OPTIONS) {
        const a = e % 16
          , i = e - a;
        t %= 128,
        s %= 128,
        this.sendMessage([midiMessageTypes.noteOn | a, t, s], i, n)
    }
    noteOff(e, t, s=!1, n=DEFAULT_SYNTH_METHOD_OPTIONS) {
        t %= 128;
        const a = e % 16
          , i = e - a;
        this._sendInternal([midiMessageTypes.noteOff | a, t], i, s, n)
    }
    stopAll(e=!1) {
        this.post({
            channelNumber: ALL_CHANNELS_OR_DIFFERENT_ACTION,
            type: "stopAll",
            data: e ? 1 : 0
        })
    }
    controllerChange(e, t, s, n=!1, a=DEFAULT_SYNTH_METHOD_OPTIONS) {
        if (t > 127 || t < 0)
            throw new Error(`Invalid controller number: ${t}`);
        s = Math.floor(s) % 128,
        t = Math.floor(t) % 128;
        const i = e % 16
          , o = e - i;
        this._sendInternal([midiMessageTypes.controllerChange | i, t, s], o, n, a)
    }
    resetControllers() {
        this.post({
            channelNumber: ALL_CHANNELS_OR_DIFFERENT_ACTION,
            type: "ccReset",
            data: null
        })
    }
    lockController(e, t, s) {
        this.post({
            channelNumber: e,
            type: "lockController",
            data: {
                controllerNumber: t,
                isLocked: s
            }
        })
    }
    channelPressure(e, t, s=DEFAULT_SYNTH_METHOD_OPTIONS) {
        const n = e % 16
          , a = e - n;
        t %= 128,
        this.sendMessage([midiMessageTypes.channelPressure | n, t], a, s)
    }
    polyPressure(e, t, s, n=DEFAULT_SYNTH_METHOD_OPTIONS) {
        const a = e % 16
          , i = e - a;
        t %= 128,
        s %= 128,
        this.sendMessage([midiMessageTypes.polyPressure | a, t, s], i, n)
    }
    pitchWheel(e, t, s=DEFAULT_SYNTH_METHOD_OPTIONS) {
        const n = e % 16
          , a = e - n;
        this.sendMessage([midiMessageTypes.pitchWheel | n, 127 & t, t >> 7], a, s)
    }
    pitchWheelRange(e, t) {
        this.controllerChange(e, midiControllers.registeredParameterMSB, 0),
        this.controllerChange(e, midiControllers.registeredParameterLSB, 0),
        this.controllerChange(e, midiControllers.dataEntryMSB, t),
        this.controllerChange(e, midiControllers.registeredParameterMSB, 127),
        this.controllerChange(e, midiControllers.registeredParameterLSB, 127),
        this.controllerChange(e, midiControllers.dataEntryMSB, 0)
    }
    programChange(e, t) {
        const s = e % 16
          , n = e - s;
        t %= 128,
        this.sendMessage([midiMessageTypes.programChange | s, t], n)
    }
    transposeChannel(e, t, s=!1) {
        this.post({
            channelNumber: e,
            type: "transposeChannel",
            data: {
                semitones: t,
                force: s
            }
        })
    }
    muteChannel(e, t) {
        this.post({
            channelNumber: e,
            type: "muteChannel",
            data: t
        })
    }
    systemExclusive(e, t=0, s=DEFAULT_SYNTH_METHOD_OPTIONS) {
        this._sendInternal([midiMessageTypes.systemExclusive, ...Array.from(e)], t, !1, s)
    }
    tuneKeys(e, t) {
        if (t.length > 127)
            throw new Error("Too many tunings. Maximum allowed is 127.");
        const s = [127, 16, 8, 2, e, t.length];
        for (const e of t)
            if (s.push(e.sourceKey),
            -1 === e.targetPitch)
                s.push(127, 127, 127);
            else {
                const t = Math.floor(e.targetPitch)
                  , n = Math.floor((e.targetPitch - t) / 61e-6);
                s.push(t, n >> 7 & 127, 127 & n)
            }
        s.push(247),
        this.systemExclusive(s)
    }
    setDrums(e, t) {
        this.post({
            channelNumber: e,
            type: "setDrums",
            data: t
        })
    }
    reverbateEverythingBecauseWhyNot() {
        for (let e = 0; e < this.channelsAmount; e++)
            this.controllerChange(e, midiControllers.reverbDepth, 127),
            this.lockController(e, midiControllers.reverbDepth, !0);
        return "That's the spirit!"
    }
    awaitWorkerResponse(e, t) {
        this.resolveMap.set(e, t)
    }
    assignProgressTracker(e, t) {
        if (this.renderingProgressTracker.get(e))
            throw new Error("Something is already being rendered!");
        this.renderingProgressTracker.set(e, t)
    }
    revokeProgressTracker(e) {
        this.renderingProgressTracker.delete(e)
    }
    _sendInternal(e, t, s=!1, n) {
        const a = fillWithDefaults(n, DEFAULT_SYNTH_METHOD_OPTIONS);
        this.post({
            type: "midiMessage",
            channelNumber: ALL_CHANNELS_OR_DIFFERENT_ACTION,
            data: {
                messageData: new Uint8Array(e),
                channelOffset: t,
                force: s,
                options: a
            }
        })
    }
    handleMessage(e) {
        switch (e.type) {
        case "eventCall":
            this.eventHandler.callEventInternal(e.data.type, e.data.data);
            break;
        case "sequencerReturn":
            this.sequencerCallbackFunction?.(e.data);
            break;
        case "isFullyInitialized":
            this.workletResponds(e.data.type, e.data.data);
            break;
        case "soundBankError":
            util.SpessaSynthWarn(e.data),
            this.eventHandler.callEventInternal("soundBankError", e.data);
            break;
        case "renderingProgress":
            this.renderingProgressTracker.get(e.data.type)?.(e.data.data)
        }
    }
    addNewChannelInternal(e) {
        this.channelProperties.push({
            voicesAmount: 0,
            pitchWheel: 0,
            pitchWheelRange: 0,
            isMuted: !1,
            isDrum: this.channelsAmount % 16 === DEFAULT_PERCUSSION,
            transposition: 0
        }),
        e && this.post({
            channelNumber: 0,
            type: "addNewChannel",
            data: null
        })
    }
    workletResponds(e, t) {
        this.resolveMap.get(e)?.(t),
        this.resolveMap.delete(e)
    }
}
  , WORKLET_PROCESSOR_NAME = "spessasynth-worklet-processor"
  , WorkletSynthesizer = class extends BasicSynthesizer {
    constructor(e, t=DEFAULT_SYNTH_CONFIG) {
        const s = fillWithDefaults(t, DEFAULT_SYNTH_CONFIG);
        let n, a = Array(18).fill(2), i = 18;
        s.oneOutput && (a = [36],
        i = 1);
        try {
            n = (s?.audioNodeCreators?.worklet ?? ((e,t,s)=>new AudioWorkletNode(e,t,s)))(e, WORKLET_PROCESSOR_NAME, {
                outputChannelCount: a,
                numberOfOutputs: i,
                processorOptions: {
                    oneOutput: s.oneOutput,
                    enableEventSystem: s.enableEventSystem
                }
            })
        } catch (e) {
            throw console.error(e),
            new Error("Could not create the AudioWorkletNode. Did you forget to addModule()?")
        }
        super(n, ((e,t=[])=>{
            n.port.postMessage(e, t)
        }
        ), s)
    }
    async startOfflineRender(e) {
        this.post({
            type: "startOfflineRender",
            data: e,
            channelNumber: -1
        }, e.soundBankList.map((e=>e.soundBankBuffer))),
        await new Promise((e=>this.awaitWorkerResponse("startOfflineRender", e)))
    }
    destroy() {
        this.reverbProcessor?.disconnect(),
        this.chorusProcessor?.delete(),
        this.post({
            channelNumber: 0,
            type: "destroyWorklet",
            data: null
        }),
        this.worklet.disconnect(),
        delete this.worklet,
        delete this.reverbProcessor,
        delete this.chorusProcessor
    }
}
  , PLAYBACK_WORKLET_PROCESSOR_NAME = "spessasynth-playback-worklet-processor";
function getPlaybackWorkletURL(e) {
    const t = new Blob([`\nconst BLOCK_SIZE = 128;\n\nconst MAX_QUEUED = ${e};\n\n/**\n * An AudioWorkletProcessor that plays back 18 separate streams of stereo audio: reverb, and chorus and 16 dry channels.\n */\nclass PlaybackProcessor extends AudioWorkletProcessor\n{\n    \n    \n    /** @type {Float32Array[]} */\n    data = [];\n    \n    updateRequested = false;\n    \n    alive = true;\n    \n    /**\n     *\n     * @type {MessagePort}\n     */\n    sentPort;\n    \n    constructor()\n    {\n        super();\n        \n        /**\n         * @param e {MessageEvent}\n         */\n        this.port.onmessage = (e) =>\n        {\n            if (e.ports.length)\n            {\n                const sentPort = e.ports[0];\n                this.sentPort = sentPort;\n                sentPort.onmessage = (e) =>\n                {\n                    if(e.data === null)\n                    {\n                        // the worklet is dead\n                        this.alive = false;\n                    }\n                    this.data.push(e.data);\n                    this.updateRequested = false;\n                    // if we need more, request immediately\n                    if (this.data.length < MAX_QUEUED)\n                    {\n                        this.sentPort.postMessage(null);\n                    }\n                };\n                \n            }\n        };\n    }\n    \n    // noinspection JSUnusedGlobalSymbols\n    /**\n     * @param _inputs {[Float32Array, Float32Array][]}\n     * @param outputs {[Float32Array, Float32Array][]}\n     * @returns {boolean}\n     */\n    process(_inputs, outputs)\n    {\n        const data = this.data.shift();\n        if (!data)\n        {\n            return this.alive;\n        }\n        let offset = 0;\n        // decode the data nicely\n        for (let i = 0; i < 18; i++)\n        {\n            outputs[i][0].set(data.subarray(offset, offset + BLOCK_SIZE));\n            offset += BLOCK_SIZE;\n            outputs[i][1].set(data.subarray(offset, offset + BLOCK_SIZE));\n            offset += BLOCK_SIZE;\n        }\n        \n        // if it has already been requested, we need to wait\n        if (!this.updateRequested)\n        {\n            this.sentPort.postMessage(null);\n            this.updateRequested = true;\n        }\n        \n        // keep it online\n        return this.alive;\n    }\n}\nregisterProcessor("${PLAYBACK_WORKLET_PROCESSOR_NAME}", PlaybackProcessor);\n    `],{
        type: "application/javascript"
    });
    return URL.createObjectURL(t)
}
import {SpessaSynthProcessor, SpessaSynthSequencer} from "spessasynth_core";
var DEFAULT_WORKER_RENDER_AUDIO_OPTIONS = {
    extraTime: 2,
    separateChannels: !1,
    loopCount: 0,
    progressCallback: void 0,
    preserveSynthParams: !0,
    enableEffects: !0
}
  , RENDER_BLOCKS_PER_PROGRESS = 64
  , BLOCK_SIZE = 128;
function renderAudioWorker(e, t) {
    const s = new SpessaSynthProcessor(e,{
        enableEventSystem: !1
    })
      , n = new SpessaSynthSequencer(s);
    this.synthesizer.soundBankManager.soundBankList.forEach((e=>s.soundBankManager.addSoundBank(e.soundBank, e.id, e.bankOffset))),
    s.soundBankManager.priorityOrder = this.synthesizer.soundBankManager.priorityOrder,
    this.stopAudioLoop();
    const a = this.sequencer.midiData;
    if (!a)
        throw new Error("No MIDI is loaded!");
    const i = this.sequencer.playbackRate
      , o = a.midiTicksToSeconds(a.loop.start) / i
      , r = a.midiTicksToSeconds(a.loop.end) / i - o
      , h = e * (a.duration / i + t.extraTime + r * t.loopCount);
    if (n.loopCount = t.loopCount,
    t.preserveSynthParams) {
        n.playbackRate = this.sequencer.playbackRate;
        const e = this.synthesizer.getSnapshot();
        s.applySynthesizerSnapshot(e)
    }
    n.loadNewSongList([a]),
    n.play();
    const l = e=>{
        this.postProgress("renderAudio", e)
    }
      , c = [new Float32Array(h), new Float32Array(h)]
      , p = [new Float32Array(h), new Float32Array(h)]
      , E = {
        reverb: c,
        chorus: p,
        dry: []
    }
      , S = h - BLOCK_SIZE;
    if (t.separateChannels) {
        const e = [];
        for (let t = 0; t < 16; t++) {
            const t = [new Float32Array(h), new Float32Array(h)];
            e.push(t),
            E.dry.push(t)
        }
        let t = 0;
        for (; ; ) {
            for (let a = 0; a < RENDER_BLOCKS_PER_PROGRESS; a++) {
                if (t >= S)
                    return n.processTick(),
                    s.renderAudioSplit(c, p, e, t, h - t),
                    this.startAudioLoop(),
                    E;
                n.processTick(),
                s.renderAudioSplit(c, p, e, t, BLOCK_SIZE),
                t += BLOCK_SIZE
            }
            l(t / h)
        }
    } else {
        const e = [new Float32Array(h), new Float32Array(h)];
        E.dry.push(e);
        let t = 0;
        for (; ; ) {
            for (let a = 0; a < RENDER_BLOCKS_PER_PROGRESS; a++) {
                if (t >= S)
                    return n.processTick(),
                    s.renderAudio(e, c, p, t, h - t),
                    this.startAudioLoop(),
                    E;
                n.processTick(),
                s.renderAudio(e, c, p, t, BLOCK_SIZE),
                t += BLOCK_SIZE
            }
            l(t / h)
        }
    }
}
var DEFAULT_BANK_WRITE_OPTIONS = {
    trim: !0,
    bankID: "",
    writeEmbeddedSoundBank: !0
}
  , DEFAULT_SF2_WRITE_OPTIONS = {
    ...DEFAULT_BANK_WRITE_OPTIONS,
    writeDefaultModulators: !0,
    writeExtendedLimits: !0,
    compress: !1,
    compressionQuality: 1,
    decompress: !1
}
  , DEFAULT_RMIDI_WRITE_OPTIONS = {
    ...DEFAULT_BANK_WRITE_OPTIONS,
    bankOffset: 0,
    correctBankOffset: !0,
    metadata: {},
    format: "sf2",
    ...DEFAULT_SF2_WRITE_OPTIONS
}
  , DEFAULT_DLS_WRITE_OPTIONS = {
    ...DEFAULT_BANK_WRITE_OPTIONS
}
  , WorkerSynthesizer = class extends BasicSynthesizer {
    timeOffset = 0;
    constructor(e, t, s=DEFAULT_SYNTH_CONFIG) {
        const n = fillWithDefaults(s, DEFAULT_SYNTH_CONFIG);
        if (n.oneOutput)
            throw new Error("One output mode is not supported in the WorkerSynthesizer.");
        let a;
        try {
            a = (n?.audioNodeCreators?.worklet ?? ((e,t,s)=>new AudioWorkletNode(e,t,s)))(e, PLAYBACK_WORKLET_PROCESSOR_NAME, {
                outputChannelCount: Array(18).fill(2),
                numberOfOutputs: 18,
                processorOptions: {
                    oneOutput: n.oneOutput,
                    enableEventSystem: n.enableEventSystem
                }
            })
        } catch (e) {
            throw console.error(e),
            new Error("Could not create the AudioWorkletNode. Did you forget to registerPlaybackWorklet()?")
        }
        super(a, t, n);
        const i = new MessageChannel
          , o = i.port1
          , r = i.port2;
        this.worklet.port.postMessage(null, [r]),
        t({
            initialTime: this.context.currentTime,
            sampleRate: this.context.sampleRate
        }, [o])
    }
    get currentTime() {
        return this.context.currentTime + this.timeOffset
    }
    static async registerPlaybackWorklet(e, t=20) {
        if (!e?.audioWorklet.addModule)
            throw new Error("Audio worklet is not supported.");
        return e.audioWorklet.addModule(getPlaybackWorkletURL(t))
    }
    handleWorkerMessage(e) {
        this.timeOffset = e.currentTime - this.context.currentTime,
        this.handleMessage(e)
    }
    async writeDLS(e=DEFAULT_DLS_WRITE_OPTIONS) {
        const t = fillWithDefaults(e, DEFAULT_DLS_WRITE_OPTIONS);
        return new Promise((s=>{
            this.assignProgressTracker("workerSynthWriteFile", (t=>{
                e.progressFunction?.(t)
            }
            ));
            const n = {
                ...t,
                progressFunction: null
            };
            this.awaitWorkerResponse("workerSynthWriteFile", (e=>s(e))),
            this.post({
                type: "writeDLS",
                data: n,
                channelNumber: -1
            })
        }
        ))
    }
    async writeSF2(e=DEFAULT_SF2_WRITE_OPTIONS) {
        const t = fillWithDefaults(e, DEFAULT_SF2_WRITE_OPTIONS);
        return new Promise((s=>{
            this.assignProgressTracker("workerSynthWriteFile", (t=>{
                e.progressFunction?.(t)
            }
            ));
            const n = {
                ...t,
                progressFunction: null
            };
            this.awaitWorkerResponse("workerSynthWriteFile", (e=>s(e))),
            this.post({
                type: "writeSF2",
                data: n,
                channelNumber: -1
            })
        }
        ))
    }
    async writeRMIDI(e=DEFAULT_RMIDI_WRITE_OPTIONS) {
        const t = fillWithDefaults(e, DEFAULT_RMIDI_WRITE_OPTIONS);
        return new Promise((s=>{
            this.assignProgressTracker("workerSynthWriteFile", (t=>{
                e.progressFunction?.(t)
            }
            ));
            const n = {
                ...t,
                progressFunction: null
            };
            this.awaitWorkerResponse("workerSynthWriteFile", (e=>s(e.binary))),
            this.post({
                type: "writeRMIDI",
                data: n,
                channelNumber: -1
            })
        }
        ))
    }
    async renderAudio(e, t=DEFAULT_WORKER_RENDER_AUDIO_OPTIONS) {
        const s = fillWithDefaults(t, DEFAULT_WORKER_RENDER_AUDIO_OPTIONS);
        if (s.enableEffects && s.separateChannels)
            throw new Error("Effects cannot be applied to separate channels.");
        return new Promise((t=>{
            this.awaitWorkerResponse("renderAudio", (async n=>{
                this.revokeProgressTracker("renderAudio");
                const a = n.reverb[0].length
                  , i = n.dry.map((t=>{
                    const s = new AudioBuffer({
                        sampleRate: e,
                        numberOfChannels: 2,
                        length: a
                    });
                    return s.copyToChannel(t[0], 0),
                    s.copyToChannel(t[1], 1),
                    s
                }
                ));
                if (!s.enableEffects)
                    return void t(i);
                const o = new AudioBuffer({
                    sampleRate: e,
                    numberOfChannels: 2,
                    length: a
                });
                o.copyToChannel(n.reverb[0], 0),
                o.copyToChannel(n.reverb[1], 1);
                const r = new AudioBuffer({
                    sampleRate: e,
                    numberOfChannels: 2,
                    length: a
                });
                r.copyToChannel(n.chorus[0], 0),
                r.copyToChannel(n.chorus[1], 1);
                const h = i[0]
                  , l = new OfflineAudioContext({
                    sampleRate: e,
                    numberOfChannels: 2,
                    length: a
                })
                  , c = l.createBufferSource();
                c.buffer = h,
                c.connect(l.destination),
                c.start();
                const p = new ChorusProcessor(l,this?.chorusProcessor?.config)
                  , E = l.createBufferSource();
                E.buffer = r,
                E.connect(p.input),
                p.connect(l.destination),
                E.start();
                const S = new ReverbProcessor(l,this.reverbProcessor?.config);
                await S.isReady;
                const d = l.createBufferSource();
                d.connect(S.input),
                S.connect(l.destination),
                d.buffer = o,
                d.start();
                const u = a / e
                  , I = setInterval((()=>{
                    s?.progressCallback?.(l.currentTime / u, 1)
                }
                ))
                  , y = await l.startRendering();
                clearInterval(I),
                t([y])
            }
            )),
            this.assignProgressTracker("renderAudio", (e=>{
                s.progressCallback?.(e, 0)
            }
            ));
            const n = {
                ...s,
                progressCallback: void 0
            };
            this.post({
                type: "renderAudio",
                data: {
                    sampleRate: e,
                    options: n
                },
                channelNumber: -1
            })
        }
        ))
    }
}
;
import {SoundBankLoader as SoundBankLoader2} from "spessasynth_core";
import {ALL_CHANNELS_OR_DIFFERENT_ACTION as ALL_CHANNELS_OR_DIFFERENT_ACTION2, BasicMIDI as BasicMIDI2, SoundBankLoader, SpessaSynthCoreUtils as util2, SpessaSynthLogging, SpessaSynthProcessor as SpessaSynthProcessor2, SpessaSynthSequencer as SpessaSynthSequencer2, SynthesizerSnapshot as SynthesizerSnapshot2} from "spessasynth_core";
import {BasicMIDI, MIDITrack} from "spessasynth_core";
var MIDIDataTrack = class extends MIDITrack {
    events = [];
    constructor(e) {
        super(),
        super.copyFrom(e),
        this.events = []
    }
}
  , MIDIData = class e extends BasicMIDI {
    tracks;
    embeddedSoundBank = void 0;
    embeddedSoundBankSize;
    constructor(t) {
        super(),
        super.copyMetadataFrom(t),
        this.tracks = t.tracks.map((e=>new MIDIDataTrack(e))),
        this.embeddedSoundBankSize = t instanceof e ? t.embeddedSoundBankSize : t?.embeddedSoundBank?.byteLength
    }
}
  , songChangeType = {
    shuffleOn: 1,
    shuffleOff: 2,
    index: 3
}
  , BasicSynthesizerCore = class {
    synthesizer;
    sequencer;
    post;
    alive = !1;
    constructor(e, t, s) {
        this.synthesizer = new SpessaSynthProcessor2(e,t),
        this.sequencer = new SpessaSynthSequencer2(this.synthesizer),
        this.post = s,
        this.synthesizer.onEventCall = e=>{
            this.post({
                type: "eventCall",
                data: e,
                currentTime: this.synthesizer.currentSynthTime
            })
        }
        ,
        this.sequencer.onEventCall = e=>{
            if ("songListChange" !== e.type)
                this.post({
                    type: "sequencerReturn",
                    data: e,
                    currentTime: this.synthesizer.currentSynthTime
                });
            else {
                const t = e.data.newSongList.map((e=>new MIDIData(e)));
                this.post({
                    type: "sequencerReturn",
                    data: {
                        type: e.type,
                        data: {
                            newSongList: t
                        }
                    },
                    currentTime: this.synthesizer.currentSynthTime
                })
            }
        }
    }
    postReady(e, t, s=[]) {
        this.post({
            type: "isFullyInitialized",
            data: {
                type: e,
                data: t
            },
            currentTime: this.synthesizer.currentSynthTime
        }, s)
    }
    postProgress(e, t) {
        this.post({
            type: "renderingProgress",
            data: {
                type: e,
                data: t
            },
            currentTime: this.synthesizer.currentSynthTime
        })
    }
    destroy() {
        this.synthesizer.destroySynthProcessor(),
        delete this.synthesizer,
        delete this.sequencer
    }
    handleMessage(e) {
        const t = e.channelNumber;
        let s;
        if (t >= 0 && (s = this.synthesizer.midiChannels[t],
        void 0 === s))
            util2.SpessaSynthWarn(`Trying to access channel ${t} which does not exist... ignoring!`);
        else
            switch (e.type) {
            case "midiMessage":
                this.synthesizer.processMessage(e.data.messageData, e.data.channelOffset, e.data.force, e.data.options);
                break;
            case "customCcChange":
                s?.setCustomController(e.data.ccNumber, e.data.ccValue);
                break;
            case "ccReset":
                t === ALL_CHANNELS_OR_DIFFERENT_ACTION2 ? this.synthesizer.resetAllControllers() : s?.resetControllers();
                break;
            case "setChannelVibrato":
                if (t === ALL_CHANNELS_OR_DIFFERENT_ACTION2)
                    for (const t of this.synthesizer.midiChannels)
                        e.data.rate === ALL_CHANNELS_OR_DIFFERENT_ACTION2 ? t.disableAndLockGSNRPN() : t.setVibrato(e.data.depth, e.data.rate, e.data.delay);
                else
                    e.data.rate === ALL_CHANNELS_OR_DIFFERENT_ACTION2 ? s?.disableAndLockGSNRPN() : s?.setVibrato(e.data.depth, e.data.rate, e.data.delay);
                break;
            case "stopAll":
                t === ALL_CHANNELS_OR_DIFFERENT_ACTION2 ? this.synthesizer.stopAllChannels(1 === e.data) : s?.stopAllNotes(1 === e.data);
                break;
            case "killNotes":
                this.synthesizer.killVoices(e.data);
                break;
            case "muteChannel":
                s?.muteChannel(e.data);
                break;
            case "addNewChannel":
                this.synthesizer.createMIDIChannel();
                break;
            case "setMasterParameter":
                this.synthesizer.setMasterParameter(e.data.type, e.data.data);
                break;
            case "setDrums":
                s?.setDrums(e.data);
                break;
            case "transposeChannel":
                s?.transposeChannel(e.data.semitones, e.data.force);
                break;
            case "lockController":
                if (e.data.controllerNumber === ALL_CHANNELS_OR_DIFFERENT_ACTION2)
                    s?.setPresetLock(e.data.isLocked);
                else {
                    if (!s)
                        return;
                    s.lockedControllers[e.data.controllerNumber] = e.data.isLocked
                }
                break;
            case "sequencerSpecific":
                {
                    if (!this.sequencer)
                        return;
                    const t = this.sequencer
                      , s = e.data;
                    switch (s.type) {
                    default:
                        break;
                    case "loadNewSongList":
                        try {
                            const e = s.data.map((e=>"duration"in e ? BasicMIDI2.copyFrom(e) : BasicMIDI2.fromArrayBuffer(e.binary, e.fileName)));
                            t.loadNewSongList(e)
                        } catch (e) {
                            console.error(e),
                            this.post({
                                type: "sequencerReturn",
                                data: {
                                    type: "midiError",
                                    data: e
                                },
                                currentTime: this.synthesizer.currentSynthTime
                            })
                        }
                        break;
                    case "pause":
                        t.pause();
                        break;
                    case "play":
                        t.play();
                        break;
                    case "setTime":
                        t.currentTime = s.data;
                        break;
                    case "changeMIDIMessageSending":
                        t.externalMIDIPlayback = s.data;
                        break;
                    case "setPlaybackRate":
                        t.playbackRate = s.data;
                        break;
                    case "setLoopCount":
                        t.loopCount = s.data;
                        break;
                    case "changeSong":
                        switch (s.data.changeType) {
                        case songChangeType.shuffleOff:
                            t.shuffleMode = !1;
                            break;
                        case songChangeType.shuffleOn:
                            t.shuffleMode = !0;
                            break;
                        case songChangeType.index:
                            void 0 !== s.data.data && (console.log("INDEX", s.data),
                            t.songIndex = s.data.data)
                        }
                        break;
                    case "getMIDI":
                        if (!t.midiData)
                            throw new Error("No MIDI is loaded!");
                        this.post({
                            type: "sequencerReturn",
                            data: {
                                type: "getMIDI",
                                data: t.midiData
                            },
                            currentTime: this.synthesizer.currentSynthTime
                        });
                        break;
                    case "setSkipToFirstNote":
                        t.skipToFirstNoteOn = s.data
                    }
                    break
                }
            case "soundBankManager":
                try {
                    const t = this.synthesizer.soundBankManager
                      , s = e.data;
                    let n;
                    switch (s.type) {
                    case "addSoundBank":
                        n = SoundBankLoader.fromArrayBuffer(s.data.soundBankBuffer),
                        t.addSoundBank(n, s.data.id, s.data.bankOffset),
                        this.postReady("soundBankManager", null);
                        break;
                    case "deleteSoundBank":
                        t.deleteSoundBank(s.data),
                        this.postReady("soundBankManager", null);
                        break;
                    case "rearrangeSoundBanks":
                        t.priorityOrder = s.data,
                        this.postReady("soundBankManager", null)
                    }
                } catch (e) {
                    this.post({
                        type: "soundBankError",
                        data: e,
                        currentTime: this.synthesizer.currentSynthTime
                    })
                }
                break;
            case "keyModifierManager":
                {
                    const t = e.data
                      , s = this.synthesizer.keyModifierManager;
                    switch (t.type) {
                    default:
                        return;
                    case "addMapping":
                        s.addMapping(t.data.channel, t.data.midiNote, t.data.mapping);
                        break;
                    case "clearMappings":
                        s.clearMappings();
                        break;
                    case "deleteMapping":
                        s.deleteMapping(t.data.channel, t.data.midiNote)
                    }
                    break
                }
            case "requestSynthesizerSnapshot":
                {
                    const e = SynthesizerSnapshot2.create(this.synthesizer);
                    this.postReady("synthesizerSnapshot", e);
                    break
                }
            case "setLogLevel":
                SpessaSynthLogging(e.data.enableInfo, e.data.enableWarning, e.data.enableGroup);
                break;
            case "destroyWorklet":
                this.alive = !1,
                this.synthesizer.destroySynthProcessor(),
                this.destroy();
                break;
            default:
                util2.SpessaSynthWarn("Unrecognized event!", e)
            }
    }
}
;
import {BasicSoundBank} from "spessasynth_core";
async function writeSF2Worker(e) {
    let t, s = this.getBank(e);
    if (e.compress && !this.compressionFunction) {
        const e = new Error("Compression enabled but no compression has been provided to WorkerSynthesizerCore.");
        throw this.post({
            type: "soundBankError",
            data: e,
            currentTime: this.synthesizer.currentSynthTime
        }),
        e
    }
    if (e.trim) {
        if (!this.sequencer.midiData)
            throw new Error("Sound bank MIDI trimming is enabled but no MIDI is loaded!");
        const e = BasicSoundBank.copyFrom(s);
        e.trimSoundBank(this.sequencer.midiData),
        s = e
    }
    void 0 !== this.compressionFunction && (t = (t,s)=>this.compressionFunction(t, s, e.compressionQuality));
    return {
        binary: await s.writeSF2({
            ...e,
            progressFunction: (e,t,s)=>(this.postProgress("workerSynthWriteFile", {
                sampleCount: s,
                sampleIndex: t,
                sampleName: e
            }),
            new Promise((e=>e()))),
            compressionFunction: t
        }),
        bank: s
    }
}
async function writeDLSWorker(e) {
    let t = this.getBank(e);
    if (e.trim) {
        if (!this.sequencer.midiData)
            throw new Error("Sound bank MIDI trimming is enabled but no MIDI is loaded!");
        const e = BasicSoundBank.copyFrom(t);
        e.trimSoundBank(this.sequencer.midiData),
        t = e
    }
    return {
        binary: await t.writeDLS({
            ...e,
            progressFunction: (e,t,s)=>(this.postProgress("workerSynthWriteFile", {
                sampleCount: s,
                sampleIndex: t,
                sampleName: e
            }),
            new Promise((e=>e())))
        }),
        bank: t
    }
}
import {BasicMIDI as BasicMIDI3} from "spessasynth_core";
async function writeRMIDIWorker(e) {
    if (!this.sequencer.midiData)
        throw new Error("No MIDI is loaded!");
    let t, s;
    if ("sf2" === e.format) {
        const n = await writeSF2Worker.call(this, e);
        s = n.binary,
        t = n.bank
    } else {
        const n = await writeDLSWorker.call(this, e);
        s = n.binary,
        t = n.bank
    }
    return BasicMIDI3.copyFrom(this.sequencer.midiData).writeRMIDI(s, {
        soundBank: t,
        ...e
    })
}
var BLOCK_SIZE2 = 128
  , WorkerSynthesizerCore = class extends BasicSynthesizerCore {
    workletMessagePort;
    compressionFunction;
    constructor(e, t, s, n) {
        super(e.sampleRate, {
            enableEventSystem: !0,
            enableEffects: !0,
            initialTime: e.initialTime
        }, s),
        this.workletMessagePort = t,
        this.workletMessagePort.onmessage = this.renderAndSendChunk.bind(this),
        this.compressionFunction = n,
        this.synthesizer.processorInitialized.then((()=>{
            this.postReady("sf3Decoder", null),
            this.startAudioLoop()
        }
        ))
    }
    handleMessage(e) {
        switch (e.type) {
        case "renderAudio":
            const t = renderAudioWorker.call(this, e.data.sampleRate, e.data.options)
              , s = [];
            t.reverb.forEach((e=>s.push(e.buffer))),
            t.chorus.forEach((e=>s.push(e.buffer))),
            t.dry.forEach((e=>s.push(...e.map((e=>e.buffer))))),
            this.postReady("renderAudio", t, s);
            break;
        case "writeRMIDI":
            this.stopAudioLoop(),
            writeRMIDIWorker.call(this, e.data).then((e=>{
                this.postReady("workerSynthWriteFile", {
                    binary: e,
                    fileName: ""
                }, [e]),
                this.startAudioLoop()
            }
            ));
            break;
        case "writeSF2":
            this.stopAudioLoop(),
            writeSF2Worker.call(this, e.data).then((e=>{
                this.postReady("workerSynthWriteFile", {
                    binary: e.binary,
                    fileName: e.bank.soundBankInfo.name + (3 === e.bank.soundBankInfo.version.major ? ".sf3" : ".sf2")
                }, [e.binary]),
                this.startAudioLoop()
            }
            ));
            break;
        case "writeDLS":
            this.stopAudioLoop(),
            writeDLSWorker.call(this, e.data).then((e=>{
                this.postReady("workerSynthWriteFile", {
                    binary: e.binary,
                    fileName: e.bank.soundBankInfo.name + ".dls"
                }, [e.binary]),
                this.startAudioLoop()
            }
            ));
            break;
        default:
            super.handleMessage(e)
        }
    }
    getBank(e) {
        let t;
        if (t = e.writeEmbeddedSoundBank && this.sequencer.midiData?.embeddedSoundBank ? SoundBankLoader2.fromArrayBuffer(this.sequencer.midiData.embeddedSoundBank) : this.synthesizer.soundBankManager.soundBankList.find((t=>t.id === e.bankID))?.soundBank,
        !t) {
            const t = new Error(`${e.bankID} does not exist in the sound bank list!`);
            throw this.post({
                type: "soundBankError",
                data: t,
                currentTime: this.synthesizer.currentSynthTime
            }),
            t
        }
        return t
    }
    stopAudioLoop() {
        this.synthesizer.stopAllChannels(!0),
        this.sequencer.pause(),
        this.alive = !1
    }
    startAudioLoop() {
        this.alive = !0,
        this.renderAndSendChunk()
    }
    destroy() {
        this.workletMessagePort.postMessage(null),
        this.stopAudioLoop(),
        super.destroy()
    }
    renderAndSendChunk() {
        if (!this.alive)
            return;
        const e = BLOCK_SIZE2 * Float32Array.BYTES_PER_ELEMENT
          , t = new Float32Array(36 * BLOCK_SIZE2);
        let s = 0;
        const n = new Float32Array(t.buffer,s,BLOCK_SIZE2);
        s += e;
        const a = [new Float32Array(t.buffer,s,BLOCK_SIZE2), n];
        s += e;
        const i = new Float32Array(t.buffer,s,BLOCK_SIZE2);
        s += e;
        const o = [i, new Float32Array(t.buffer,s,BLOCK_SIZE2)]
          , r = [];
        for (let n = 0; n < 16; n++) {
            s += e;
            const n = new Float32Array(t.buffer,s,BLOCK_SIZE2);
            s += e;
            const a = new Float32Array(t.buffer,s,BLOCK_SIZE2);
            r.push([n, a])
        }
        this.sequencer.processTick(),
        this.synthesizer.renderAudioSplit(a, o, r),
        this.workletMessagePort.postMessage(t, [t.buffer])
    }
}
;
import {ALL_CHANNELS_OR_DIFFERENT_ACTION as ALL_CHANNELS_OR_DIFFERENT_ACTION3, BasicMIDI as BasicMIDI4, midiMessageTypes as midiMessageTypes2, SpessaSynthCoreUtils as SpessaSynthCoreUtils4} from "spessasynth_core";
var DEFAULT_SEQUENCER_OPTIONS = {
    skipToFirstNoteOn: !0,
    initialPlaybackRate: 1
}
  , SeqEventHandler = class {
    timeDelay = 0;
    events = {
        songChange: new Map,
        songEnded: new Map,
        metaEvent: new Map,
        timeChange: new Map,
        midiError: new Map,
        textEvent: new Map
    };
    addEvent(e, t, s) {
        this.events[e].set(t, s)
    }
    removeEvent(e, t) {
        this.events[e].delete(t)
    }
    callEventInternal(e, t) {
        const s = this.events[e]
          , n = ()=>{
            s.forEach((s=>{
                try {
                    s(t)
                } catch (t) {
                    console.error(`Error while executing a sequencer event callback for ${e}:`, t)
                }
            }
            ))
        }
        ;
        this.timeDelay > 0 ? setTimeout(n.bind(this), 1e3 * this.timeDelay) : n()
    }
}
  , Sequencer = class {
    midiData;
    songListData = [];
    eventHandler = new SeqEventHandler;
    isFinished = !1;
    synth;
    midiOut;
    isLoading = !1;
    pausedTime = 0;
    getMIDICallback = void 0;
    highResTimeOffset = 0;
    absoluteStartTime;
    constructor(e, t=DEFAULT_SEQUENCER_OPTIONS) {
        this.synth = e,
        this.absoluteStartTime = this.synth.currentTime,
        this.synth.sequencerCallbackFunction = this.handleMessage.bind(this),
        this._skipToFirstNoteOn = t?.skipToFirstNoteOn ?? !0,
        1 !== t?.initialPlaybackRate && (this.playbackRate = t?.initialPlaybackRate ?? 1),
        this._skipToFirstNoteOn || this.sendMessage("setSkipToFirstNote", !1),
        window.addEventListener("beforeunload", this.resetMIDIOutput.bind(this))
    }
    _songIndex = 0;
    get songIndex() {
        return this._songIndex
    }
    set songIndex(e) {
        const t = Math.max(0, e % this._songsAmount);
        t !== this._songIndex && (this.isLoading = !0,
        this.midiData = void 0,
        this.sendMessage("changeSong", {
            changeType: songChangeType.index,
            data: t
        }))
    }
    _currentTempo = 120;
    get currentTempo() {
        return this._currentTempo
    }
    get duration() {
        return this.midiData?.duration ?? 0
    }
    _songsAmount = 0;
    get songsAmount() {
        return this._songsAmount
    }
    _skipToFirstNoteOn;
    get skipToFirstNoteOn() {
        return this._skipToFirstNoteOn
    }
    set skipToFirstNoteOn(e) {
        this._skipToFirstNoteOn = e,
        this.sendMessage("setSkipToFirstNote", this._skipToFirstNoteOn)
    }
    _loopCount = -1;
    get loopCount() {
        return this._loopCount
    }
    set loopCount(e) {
        this._loopCount = e,
        this.sendMessage("setLoopCount", e)
    }
    _playbackRate = 1;
    get playbackRate() {
        return this._playbackRate
    }
    set playbackRate(e) {
        const t = this.currentTime;
        this.sendMessage("setPlaybackRate", e),
        this.highResTimeOffset *= e / this._playbackRate,
        this._playbackRate = e,
        this.recalculateStartTime(t)
    }
    _shuffleSongs = !1;
    get shuffleSongs() {
        return this._shuffleSongs
    }
    set shuffleSongs(e) {
        this._shuffleSongs = e,
        e ? this.sendMessage("changeSong", {
            changeType: songChangeType.shuffleOn
        }) : this.sendMessage("changeSong", {
            changeType: songChangeType.shuffleOff
        })
    }
    get currentTime() {
        return this.isLoading ? 0 : void 0 !== this.pausedTime ? this.pausedTime : (this.synth.currentTime - this.absoluteStartTime) * this._playbackRate
    }
    set currentTime(e) {
        this.sendMessage("setTime", e)
    }
    get currentHighResolutionTime() {
        if (void 0 !== this.pausedTime)
            return this.pausedTime;
        const e = this.highResTimeOffset
          , t = this.absoluteStartTime
          , s = (performance.now() / 1e3 - t) * this._playbackRate;
        let n = e + s;
        const a = this.currentTime
          , i = .01 * this._playbackRate
          , o = a - n;
        return this.highResTimeOffset += o * i,
        n = this.highResTimeOffset + s,
        n
    }
    get paused() {
        return void 0 !== this.pausedTime
    }
    async getMIDI() {
        return new Promise((e=>{
            this.getMIDICallback = e,
            this.sendMessage("getMIDI", null)
        }
        ))
    }
    loadNewSongList(e) {
        this.isLoading = !0,
        this.midiData = void 0,
        this.sendMessage("loadNewSongList", e),
        this._songIndex = 0,
        this._songsAmount = e.length
    }
    connectMIDIOutput(e) {
        this.resetMIDIOutput(),
        this.midiOut = e,
        this.sendMessage("changeMIDIMessageSending", void 0 !== e),
        this.currentTime -= .1
    }
    pause() {
        this.paused || (this.pausedTime = this.currentTime,
        this.sendMessage("pause", null))
    }
    play() {
        this.recalculateStartTime(this.pausedTime ?? 0),
        this.pausedTime = void 0,
        this.isFinished = !1,
        this.sendMessage("play", null)
    }
    handleMessage(e) {
        switch (e.type) {
        case "midiMessage":
            const t = e.data.message;
            if (this.midiOut && t[0] >= 128)
                return void this.midiOut.send(t);
            break;
        case "songChange":
            this._songIndex = e.data.songIndex;
            const s = this.songListData[this._songIndex];
            this.midiData = s,
            this.isLoading = !1,
            this.absoluteStartTime = 0,
            this.callEventInternal("songChange", s);
            break;
        case "timeChange":
            const n = e.data.newTime;
            this.recalculateStartTime(n),
            this.callEventInternal("timeChange", n);
            break;
        case "pause":
            this.pausedTime = this.currentTime,
            this.isFinished = e.data.isFinished,
            this.isFinished && this.callEventInternal("songEnded", null);
            break;
        case "midiError":
            throw this.callEventInternal("midiError", e.data),
            new Error(`MIDI parsing error:  ${e.data}`);
        case "getMIDI":
            this.getMIDICallback && this.getMIDICallback(BasicMIDI4.copyFrom(e.data));
            break;
        case "metaEvent":
            const a = e.data.event;
            switch (a.statusByte) {
            case midiMessageTypes2.setTempo:
                this._currentTempo = 6e7 / SpessaSynthCoreUtils4.readBytesAsUintBigEndian(a.data, 3);
                break;
            case midiMessageTypes2.text:
            case midiMessageTypes2.lyric:
            case midiMessageTypes2.copyright:
            case midiMessageTypes2.trackName:
            case midiMessageTypes2.marker:
            case midiMessageTypes2.cuePoint:
            case midiMessageTypes2.instrumentName:
            case midiMessageTypes2.programName:
                if (!this.midiData)
                    break;
                let e = -1;
                a.statusByte === midiMessageTypes2.lyric && (e = Math.min(this.midiData.lyrics.findIndex((e=>e.ticks === a.ticks)), this.midiData.lyrics.length - 1)),
                !this.midiData.isKaraokeFile || a.statusByte !== midiMessageTypes2.text && a.statusByte !== midiMessageTypes2.lyric || (e = Math.min(this.midiData.lyrics.findIndex((e=>e.ticks === a.ticks)), this.midiData.lyrics.length)),
                this.callEventInternal("textEvent", {
                    event: a,
                    lyricsIndex: e
                })
            }
            this.callEventInternal("metaEvent", {
                event: e.data.event,
                trackNumber: e.data.trackIndex
            });
            break;
        case "loopCountChange":
            this._loopCount = e.data.newCount,
            this._loopCount;
            break;
        case "songListChange":
            this.songListData = e.data.newSongList.map((e=>new MIDIData(e))),
            this.midiData = this.songListData[this._songIndex]
        }
    }
    callEventInternal(e, t) {
        this.eventHandler.callEventInternal(e, t)
    }
    resetMIDIOutput() {
        if (this.midiOut) {
            for (let e = 0; e < 16; e++)
                this.midiOut.send([midiMessageTypes2.controllerChange | e, 120, 0]),
                this.midiOut.send([midiMessageTypes2.controllerChange | e, 123, 0]);
            this.midiOut.send([midiMessageTypes2.reset])
        }
    }
    recalculateStartTime(e) {
        this.absoluteStartTime = this.synth.currentTime - e / this._playbackRate,
        this.highResTimeOffset = (this.synth.currentTime - performance.now() / 1e3) * this._playbackRate,
        this.paused && (this.pausedTime = e)
    }
    sendMessage(e, t) {
        this.synth.post({
            channelNumber: ALL_CHANNELS_OR_DIFFERENT_ACTION3,
            type: "sequencerSpecific",
            data: {
                type: e,
                data: t
            }
        })
    }
}
;
import {audioToWav} from "spessasynth_core";
function audioBufferToWav(e, t) {
    const s = []
      , n = t?.channelOffset ?? 0
      , a = t?.channelCount ?? e.numberOfChannels;
    for (let t = n; t < e.numberOfChannels && (s.push(e.getChannelData(t)),
    !(s.length >= a)); t++)
        ;
    return new Blob([audioToWav(s, e.sampleRate, t)],{
        type: "audio/wav"
    })
}
import {SpessaSynthCoreUtils as util3} from "spessasynth_core";
var LibMIDIPort = class {
    port;
    constructor(e) {
        this.port = e
    }
    get id() {
        return this.port.id
    }
    get name() {
        return this.port.name
    }
    get manufacturer() {
        return this.port.manufacturer
    }
    get version() {
        return this.port.version
    }
}
  , LibMIDIInput = class extends LibMIDIPort {
    connectedSynths = new Set;
    constructor(e) {
        super(e),
        e.onmidimessage = e=>this.connectedSynths.forEach((t=>{
            e.data && t.sendMessage(e.data)
        }
        ))
    }
    connect(e) {
        this.connectedSynths.add(e)
    }
    disconnect(e) {
        this.connectedSynths.delete(e)
    }
}
  , LibMIDIOutput = class extends LibMIDIPort {
    port;
    constructor(e) {
        super(e),
        this.port = e
    }
    connect(e) {
        e.connectMIDIOutput(this.port)
    }
    disconnect(e) {
        e.connectMIDIOutput(void 0)
    }
}
  , MIDIDeviceHandler = class e {
    inputs = new Map;
    outputs = new Map;
    constructor(e) {
        e.inputs.forEach(((e,t)=>{
            this.inputs.set(t, new LibMIDIInput(e))
        }
        )),
        e.outputs.forEach(((e,t)=>{
            this.outputs.set(t, new LibMIDIOutput(e))
        }
        ))
    }
    static async createMIDIDeviceHandler() {
        if (!navigator.requestMIDIAccess)
            throw util3.SpessaSynthWarn("Web MIDI API is not supported.", consoleColors.unrecognized),
            new Error("Web MIDI API is not supported.");
        try {
            const t = await navigator.requestMIDIAccess({
                sysex: !0,
                software: !0
            });
            return util3.SpessaSynthInfo("%cMIDI handler created!", consoleColors.recognized),
            new e(t)
        } catch (e) {
            throw util3.SpessaSynthWarn("Could not get MIDI Devices:", e),
            e
        }
    }
}
;
import {SpessaSynthCoreUtils as SpessaSynthCoreUtils5} from "spessasynth_core";
var WebMIDILinkHandler = class {
    constructor(e) {
        window.addEventListener("message", (t=>{
            if ("string" != typeof t.data)
                return;
            const s = t.data.split(",");
            if ("midi" !== s[0])
                return;
            s.shift();
            const n = s.map((e=>parseInt(e, 16)));
            e.sendMessage(n)
        }
        )),
        SpessaSynthCoreUtils5.SpessaSynthInfo("%cWeb MIDI Link handler created!", consoleColors.recognized)
    }
}
;
export {ChorusProcessor, DEFAULT_SYNTH_CONFIG, MIDIDeviceHandler, ReverbProcessor, Sequencer, WebMIDILinkHandler, WorkerSynthesizer, WorkerSynthesizerCore, WorkletSynthesizer, audioBufferToWav};
//# sourceMappingURL=/sm/95b979934ad9fb86554e27ad2ebcc63661b8a7a30f2be8d8921ac7be1b6f6fbf.map
